...
вывести_всех_животных :- 
    ...
    (сотрудничество(Животное, Партнер) -> format('  Сотрудничество с: ~w~n', [Партнер]) ; true),
    (требует_дополнительного_корма(Животное, Причина) -> format('  Требует дополнительного корма: ~w~n', [Причина]) ; true),
